...
function cheerpjReportJSLoadReason(loadedName, callback)
{
	...
	for(var i=contStack.length-1;i>=0;i--)
	{
		...
		else
		{
			...
			if(scriptName && !scriptName.startsWith("runtime/"))
			{
...
function runContinuationStack(ret)
{
	...
	catch(e)
	{
		...
		{
			assert(sSlot);
			currentThread.continuationStack.push({func:function(args)
				{
					var ex=new N4java4lang20NullPointerException();
					// HACK: Attach the raw JS stack, it helps automatically reported errors
					ex.jsStack = e.stack;
					// TODO: Stack trace, constructor
					cheerpjThrow(args, ex);
					assert(args=null);
				}, args: sSlot});
			currentThread.continuationStack.push({func:function(args)
				{
					cheerpjEnsureInitialized("N4java4lang20NullPointerException", null);
				}, args: null});
		}
...
function cheerpjCallStaticFunc(funcName)
{
	try
	{
		window[funcName]({});
...
// TODO: We should really not encode them as JS string
var internStringMap = {}
...
function cjC(c,p)
{
	...
	if(!window.hasOwnProperty(mangledClassName))
	{
...
function cheerpjInitOnce()
{
	...
	cjGlobal.addEventListener("error", function(err)
	{
		var message = "Error";
		if(err)
		{
...
function cheerpjDoTimer()
{
	...
	assert(thisThread.state == "BLOCKED_SLEEP");
...
function cheerpjRewriteAndReplaceApplet(elem)
{
		...
		newScript.onload = function()
		{
			...
			xhr.onload=function(e)
			{
				...
				cheerpjCreateInstance("N3com11leaningtech7cheerpj19CheerpJAppletViewer", "V").then(function(a) { 
					...
					cheerpjRunMethod(a, "v529", retData.mainClass, loc, window.location.href, computedWidth, computedHeight, newObj, []);
				});
			}
	...
	cheerpjCreateInstance("N3com11leaningtech7cheerpj19CheerpJAppletViewer", "V").then(function(a) { 
		...
		// startAppletN4java4lang6StringN4java4lang6StringIIN4java4lang6ObjectN4java4lang6ObjectEV
		cheerpjRunMethod(a, "v529", appletClass, loc, window.location.href, computedWidth, computedHeight, newObj, appletParameters);
	});
}
...
function cheerpjInit(options)
{
	...
	var lateScripts = options && options.lateScripts ? options.lateScripts : null;
	...
	if(query[0] == "?")
	{
		...
		for(var i=0;i<parts.length;i++)
		{
			...
			if(keyValue[0] == "cheerpjJarJsOverridePath")
				cheerpjJarJsOverridePath = keyValue[1];
	...
	if(lateScripts)
		cheerpjLateScripts = lateScripts;
...
function cheerpjJNIBridge(args, instance)
{
	...
	var jniFunc = window[jniFuncName];
	assert(jniFunc);
...
function cheerpjEnsureInitialized(mangledName, p)
{
	...
	assert(window.hasOwnProperty(mangledName));
...
function cheerpjThrow(a, ex)
{
	...
	while(curA)
	{
		...
		if(!window.hasOwnProperty(exFuncName))
		{
			curA = curA.p;
			continue;
		}
...
function cheerpjGetJSFromClassName(mangledName)
{
	...
	var classConstructor = window[mangledName+"X"];
	if(!classConstructor)
		return null;
	...
	catch(e)
	{
		...
		var tmp=part.split("http://");
		if(tmp.length < 2)
			tmp=part.split("https://");
		var path=tmp[1];
		var parts = path.split('/');
		parts.shift();
		return parts.join('/');
...
function cheerpjArrayInstanceof(obj, arrayType, p)
{
	...
	if(!objConstructorFunc || !window[objConstructorFunc.name+'G'])
	{
...
{
	var cinit=window[mangledClassName + 'X'];
...
function cjMethodDynamic(name, compressHint, suffix)
{
	...
	if(suffix)
		plainName += suffix;
	if(window.hasOwnProperty("_"+plainName))
		return window["_"+plainName];
	// Try the compressed one otherwise
	var compressedName = cheerpjCompressSymbol(name);
	if(suffix)
		compressedName += suffix;
	return window["_"+compressedName];
}
...
function cjHasGlobalDynamic(name)
{
	...
	// Try the compressed one otherwise
	var compressedName = cheerpjCompressSymbol(name);
	return window.hasOwnProperty(compressedName);
}
...
function cheerpjSetStatus(s, elem)
{
	...
	if(cjStatus == null)
		return;
	cjStatus = s;
	var display = elem ? elem : document.getElementById("cheerpjDisplay");
	if(display)
	{
